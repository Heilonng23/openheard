...
// One image per request, the raw file as the body. Signed-in users only, in
// the workspace the host names; the upload stays unclaimed until the post or
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const limit = await rateLimit(`upload:${user.id}`, { window: 600, max: 40 });
        if (!limit.allowed) return json({ error: "Too many uploads. Try again in a few minutes." }, 429, { "retry-after": String(limit.retryAfter) });
